...
class VisitCountries extends Component {
  ...
  onClickButton = id => {
    ...
    const visitCountry = countriesList.map(each => {
      if (each.id === id) {
  ...
  onClickDeleteButton = id => {
    ...
    const updateCountry = countriesList.map(each => {
      if (id === each.id) {
        const data = {...each, isVisited: !each.isVisited}
        return data
      }
      return each
    })
    const updatedData = countriesList.filter(eachData => eachData.id !== id)

    this.setState({countriesList: updatedData})
    this.setState({countriesList: updateCountry})
  }

  render() {
    const {countriesList} = this.state

    const filteredData = countriesList.filter(
      eachOne => eachOne.isVisited === true,
    )

    return (
      <div className="visited-countries-bg-container">
        <div className="visited-countries-container">
          ...
          {filteredData.length > 0 ? (
            <ul className="countrys-list-container">
